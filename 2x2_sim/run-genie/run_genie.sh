...
# Convert to ROOTRacker format
run gntpc -i "$genieOutPrefix".GHEP.root -f rootracker \
    -o "$genieOutPrefix".GTRAC.root

# Convert to GENIE summary tree (gst)
run gntpc -i "$genieOutPrefix".GHEP.root -f gst \
    -o "$genieOutPrefix".gst.root

# Make output directories
mkdir -p "$outDir/GHEP/$subDir"  "$outDir/GTRAC/$subDir" "$outDir/GST/$subDir"
...
mv "$genieOutPrefix.gst.root"  "$outDir/GST/$subDir"
